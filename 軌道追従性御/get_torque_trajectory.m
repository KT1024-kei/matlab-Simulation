...
for t=0:0.01:Ts
    ...
    x = [sin(omega*t);cos(omega*t);0];
    v = [omega*cos(omega*t);-omega*sin(omega*t);0];
    a = [-omega^2*sin(omega*t);-omega^2*cos(omega*t);0];
    j = [-omega^3*cos(omega*t);omega^3*sin(omega*t);0];
    s = [omega^4*sin(omega*t);omega^4*cos(omega*t);0];
...
%% 角速度
t=0:0.01:Ts;
plot(t, W(:, 1), 'LineWidth', 3)
hold on
plot(t, W(:, 2), 'LineWidth', 3)
plot(t, W(:, 3), 'LineWidth', 3)

grid on
xlabel('T[s]'); ylabel('Moment');
set(gca, 'FontSize', 15, 'FontName', 'Times')
legend({'W1', 'W2', 'W3'}, 'Location', 'NorthEast', 'Interpreter', 'latex');
hold off
%% モーメントプロット
t=0:0.01:Ts;
plot(t, M(:, 1), 'LineWidth', 3)
hold on
plot(t, M(:, 2), 'LineWidth', 3)
plot(t, M(:, 3), 'LineWidth', 3)
